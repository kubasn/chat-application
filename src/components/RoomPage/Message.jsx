    ...
    <MessageComponent
      ...
      left={
        user.login === message.senderName
          ...
          : anotherUserMessage.left
      }
      color={
        message.content === messageDeletedContent
          ? "transparent"
          : user.login === message.senderName
          ? currentUserMessage.bgColor
          : anotherUserMessage.bgColor
      }
      style={{
        border:
          message.content === messageDeletedContent && "1px solid #ededed",
      }}
    >
      <MessageHeader>
        <MessageSender
          color={
            user.login === message.senderName
              ? currentUserMessage.color
              : anotherUserMessage.color
          }
        >
          {message.senderName}
        </MessageSender>
      ...
      <MessageContent>
        <Typography
          sx={{ color: message.content !== messageDeletedContent && "black" }}
          variant="body2"
        >
          {message.content}
        </Typography>
